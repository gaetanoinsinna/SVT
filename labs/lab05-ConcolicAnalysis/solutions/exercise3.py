...
def main(argv):
  ...
  # (!)
  password0_address = 0x09413560
  initial_state.memory.store(password0_address, password0)
  password1_address = 0x09413568
  initial_state.memory.store(password1_address, password1)
  password2_address = 0x09413570
  initial_state.memory.store(password2_address, password2)
  password3_address = 0x09413578
  ...
  if simulation.found:
    ...
    solution1 = solution_state.se.eval(password1,cast_to=bytes).decode()
    solution2 = solution_state.se.eval(password2,cast_to=bytes).decode()
    solution3 = solution_state.se.eval(password3,cast_to=bytes).decode()
    solution = " ".join(map(str, [solution0, solution1, solution2, solution3]))

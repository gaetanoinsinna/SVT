...
def main(argv):
  ...
  password_size_in_bits = 16 * 8
  password = claripy.BVS('password', password_size_in_bits)

  #it is the address where the scanf save the password => we want to match the password to the desired value
  ...
  #the address where the check function is called
  ...
  if simulation.found:
    ...
    # (!)
    constrained_parameter_desired_value = b"XGDEOCCPTSBOPWCE" # :string
